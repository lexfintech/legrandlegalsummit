...
const EventAgenda = ({ agendaDetails, date, location }: { agendaDetails: Agenda[], date: string, location: string }) => {
  if (agendaDetails.length === 0) return null;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center mb-12">
          <div className="bg-background-primary p-2 rounded-lg">
            {agendaDetails.map((day, index) => (
              <button
                onClick={() => setActiveDay(index)}
                className={`px-4 md:px-8 py-3 rounded-lg font-semibold transition-all duration-300 ${activeDay === index
        ...
        <div className="space-y-6">
          {agendaDetails[activeDay].sessions.map(
            (session, index) => (
              <div
                key={index}
